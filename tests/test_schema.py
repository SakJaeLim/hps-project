...
def test_contracts_instantiate():
    ...
    c = Container(id="1", weight_ton=24.5, size="40", type="GP", pod="LAX", dg=False)
    s = Slot(bay=3, row=2, tier=4, max_stack_weight=30.0, dg_allowed=True)
    ys = YardState(slots=[s], queue=[c])
    cp = CandidatePlan(assignments=[Assignment("1", 3, 2, 4)], engine="greedy")
    rec = Recommendation(plan=cp, violations=[Violation(rule="stack_weight", container_id="1")],
                         rationale="ok", checks=["stack_weight"])
    assert ys.queue[0].id == "1"
